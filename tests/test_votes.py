...
def test_delete_vote_na(authorized_client, test_posts):
    res = authorized_client.post("/vote/", 
                                json={"post_id": test_posts[0].id, "dir": 0})
    assert res.status_code == 404

def test_vote_na_post(authorized_client, test_posts):
    res = authorized_client.post("/vote/", 
                                json={"post_id": 8000, "dir": 1})
    assert res.status_code == 404
...
def test_vote_unathorized_user(client, test_posts, test_vote):
