...
def test_get_all_posts(authorized_client, test_posts):
    ...
    assert res.status_code == 201 #changed this one
...
def test_na_post(authorized_client, test_posts):
...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("testing create", "oh wow it worked", False),
    ("testing another time", "still worked, epic", True)
    ])
...
def test_create_post_def_pub(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title": "something", "content": "something else"})

    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "something"
    assert created_post.content == "something else"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unauthorized_user_Create_post(client, test_posts, test_user):
    res = client.post("/posts/", json={"title": "something", "content": "something else"})
    assert res.status_code == 401
...
def test_delete_na_post(authorized_client, test_user, test_posts):
...
def test_update_na_post(authorized_client, test_user, test_posts):
    data = {
        "title": "updated title",
        "content": "updated content",
        "id": test_posts[3].id
